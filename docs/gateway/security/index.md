...
- `tools.fs.allowedDirectories: [...]` (optional): restricts filesystem tool paths to a specific list of directories. When specified, this takes precedence over `workspaceOnly`. Supports both absolute and relative paths (relative to workspace root). Useful for multi-project access control.
